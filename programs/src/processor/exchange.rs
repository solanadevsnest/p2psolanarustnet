...
pub fn handler_exchange(ctx: Context<Exchange>) -> Result<()> {
    ...
    let incoming_amount = ctx.accounts.escrow_state.incoming_amount;
    //
    ...
    match trade_type {
        ...
        Ok(TradeType::TokenToken) => {
            ...
            ctx.accounts.transfer_tokens_to_recipient(trade_value)?;
            // transfer TOKEN: partner -> Creator
            ctx.accounts.transfer_to_creator_token(incoming_amount)?;
        ...
        Ok(TradeType::TokenSol) => {
            ...
            ctx.accounts.transfer_to_creator_native(incoming_amount)?;
            ctx.accounts.close_vault_token()?;
        }
        // Case SOL - SPL
...
impl<'info> Exchange<'info> {
    ...
    fn transfer_tokens_to_recipient(&self, amount: u64) -> Result<()> {
        let creator_key = self.creator.key();
        let order_id_bytes = self.escrow_state.order_id.to_le_bytes();
